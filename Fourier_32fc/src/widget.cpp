...
long double interpolate( long double curSampTime, double oldSampValArr[], double DCT[], qint32 numOfSamp ){
    ...
    if( inFile.open( QIODevice::ReadOnly ) && outFile.open( QIODevice::WriteOnly ) ){
        ...
        if( numOfChannels == 1 ){
            ...
            for (int i = 0; i < sampCount; ++i) {
                inBufMonoDouble[i] = (double) inBufMono[i];
            }
            fftw_execute(plan);
            DCT[0] *= 0.5L;
            while ( curSampTime < MAX_TIME ) {
                float outSamp = (float) interpolate( curSampTime, inBufMonoDouble, DCT, std::min( sampCount, inBufSize ) );
            ...
            sampCount = inFile.read((char*)inBufStereo, inBufSize*2*sizeof(float)) / sizeof(float);
            for (int i = 0; i < sampCount; i++) {
                i % 2 == 0 ? inBufLeft[i >> 1]       = (double) inBufStereo[i]
                           : inBufRight[ (i-1) >> 1] = (double) inBufStereo[i];
            }
            ...
            while ( curSampTime < MAX_TIME ) {
                float outSampLeft = (float) interpolate( curSampTime, inBufLeft, DCT_Left, std::min( sampCount, inBufSize ) );
                outFile.write( (char*)&outSampLeft, sizeof(float) );
                float outSampRight = (float) interpolate( curSampTime, inBufRight, DCT_Right, std::min( sampCount, inBufSize ) );
                outFile.write( (char*)&outSampRight, sizeof(float) );
                totalSamps += 2;
                curSampTime += ratio;
            }
            ui->infoPRB->setValue( 100 * inFile.pos() / inFile.size() );
            qApp->processEvents();
            while(!inFile.atEnd()){
                inFile.seek( inFile.pos() - overlap * 2 * 2 * sizeof(float) );
                curSampTime -= ( NUM_OF_R_SAMPS );
                sampCount = inFile.read((char*)inBufStereo, inBufSize*2*sizeof(float)) / sizeof(float);
                for (int i = 0; i < sampCount; i ++) {
                    i % 2 == 0 ? inBufLeft[i >> 1]       = (double) inBufStereo[i]
        ...
        if( fileExt == "wav" ){
            quint32 subchunk2size = totalSamps  * 4;
